"""Factory class for creating environments and agents."""
...
class BanditFactory:
    """Factory for creating bandit environments and agents."""
    ...
    def make_agent(
        ...
        **kwargs,
        ...
        if name not in cls._AGENT_REGISTRY:
            raise ValueError(
                f"Unknown agent '{name}'. Options: {list(cls._AGENT_REGISTRY)}"
            )

        agent = cls._AGENT_REGISTRY[name](k=k, seed=seed, **kwargs)
